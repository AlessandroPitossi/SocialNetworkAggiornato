...
public class Interfaces {
	...
	public static String mostraEvento(Concerto e) {
		String s=mostraEvento((Evento)e);
		s+=COSTI_AGGIUNTIVI+"\n";
		if(e.getTshirt()!=0.0) s+="--"+Concerto.TSHIRT+": "+e.getTshirt()+"\n";
		if(e.getPass()!=0.0) s+="--"+Concerto.PASS_BACKSTAGE+": "+e.getPass()+"\n\n";
		s+=Concerto.ARTISTA+": "+e.getArtista()+"\n";
		return s;
	}
	...
	public static Notifica invito(Evento e) {
		...
		String descrizione=DESCRIZIONE_INVITO1+u.getId()+DESCRIZIONE_INVITO2+e.getNome()+DESCRIZIONE2+
				mylib.Utility.DateToString(e.getValueCampoDate(Evento.DATA))+DESCRIZIONE_INVITO3+e.mostraEv();
		return new Notifica(titolo, descrizione, NUOVA, e.getColumn(), INVITO);
	}
}
