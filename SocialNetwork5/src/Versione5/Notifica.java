...
public class Notifica {
	...
	private static final String QUOTA_AGGIUNTIVA="\nLa spesa aggiuntiva da pagare per spese extra è: ";
	private static final String QUOTA_TOTALE="\nLa quota totale da pagare è: ";
	...
	public Notifica(Evento e, double quotaAggiuntiva) {
		titolo="";
		...
		if(e.getStato().equals(Stato.FALLITA)) {
			...
			if(e.getStato().equals(Stato.RITIRATA)) descrizione=EVENTO_RITIRATO;
				...
				double quota=(double) e.getCampo(Evento.QUOTA).getValore()+quotaAggiuntiva;
				if (quota==0){
					descrizione+=QUOTA_NULLA;
				}
				else {
					descrizione+=QUOTA_EVENTO+e.getCampo(Evento.QUOTA).getValore()+EURO;
					if(quotaAggiuntiva!=0) {
						descrizione+=QUOTA_AGGIUNTIVA+quotaAggiuntiva+EURO;
						descrizione+=QUOTA_TOTALE+quota+EURO;
					}
				}
			}
		}
